...
```{r include = FALSE}
knitr::opts_chunk$set(message = FALSE, warning = FALSE, fig.path = "../ohio/figs")
```
...
```{r include = FALSE}
noble <- read_csv("../ohio/noble_geo.csv")
```
...
```{r echo = FALSE}
pics <- list.files(path = "../ohio/figs/gif", pattern = "*.png", full.names = T) %>% 
  map(image_read) %>% # reads each path file
  image_join() %>% # joins image
  image_animate(fps=1) %>% # animates, can opt for number of loops
  image_write("../ohio/figs/iterations2.gif") # write to current dir
